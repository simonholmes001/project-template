...
{{PROJECT_NAME}} starts with a deliberately small Azure footprint:
...
- `rg-{{REPO_NAME}}-pipeline-identity`
- `id-{{REPO_NAME}}-github-actions`
- GitHub OIDC federated credential for `{{GITHUB_REF}}` using GitHub's immutable owner/repository subject format
- `rg-{{REPO_NAME}}-dev`
- `Contributor` assignment for the pipeline identity on `rg-{{REPO_NAME}}-dev`
...
```bash
AZURE_CONFIG_DIR=/tmp/{{REPO_NAME}}-azure ./infrastructure/scripts/validate.sh dev --lint-only
```

Full validation and what-if require Azure login:

```bash
AZURE_CONFIG_DIR=/tmp/{{REPO_NAME}}-azure \
