...
render_stream() {
  perl -pe '
    ...
    s/\{\{GITHUB_OWNER_ID\}\}/$ENV{GITHUB_OWNER_ID}/g;
    s/\{\{GITHUB_REPO_ID\}\}/$ENV{GITHUB_REPO_ID}/g;
    s/\{\{GITHUB_REF\}\}/$ENV{GITHUB_REF}/g;
...
prepare_context() {
  ...
  export REPO_NAME REPO_NAME_UNDERSCORE GITHUB_OWNER GITHUB_OWNER_ID GITHUB_REPO_ID GITHUB_REF
  export ENV_PREFIX AZURE_LOCATION
